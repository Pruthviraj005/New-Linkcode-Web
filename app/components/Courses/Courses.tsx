...
const NamesList = () => {
    ...
    const nameElements = selectedNames.map((name, index) => (
        ...
        <div key={index}>
            <div className=" text-lg sm:text-sm py-5 lg:py-0 hover:shadow-lg">
                ...
                <div className='flex justify-between border-solid border-2 border-grey500 rounded-md p-2'>
                    ...
                    <div className='flex flex-row space-x-4'>
                        <div className='flex'>
                            <Image width={0} height={0} src={'/assets/courses/account.svg'} alt="circle" />
                            <p className='text-lightgrey ml-1'>120</p>
                        </div>
                        <div className='flex'>
                            <Image width={0} height={0} src={'/assets/courses/Star.svg'} alt="star" />
        ...
        <div className="-mt-14">
            <div id='courses-section' className="mx-auto max-w-2xl py-16 px-4 sm:py-36 sm:px-6 lg:max-w-7xl lg:px-8">
                ...
                <div className='sm:flex justify-between items-center pb-12'>
                    <h2 className="text-3xl sm:text-5xl font-bold tracking-tight text-blue-900 my-4">Popular Courses</h2>
                    <div>
                        <Button
                            className="bg-white text-blue-900 font-semibold tracking-wider hover:bg-green-50"
                        >
                            Explore Courses
                        </Button>
                    </div>
                ...
                <div className='flex nowhitespace space-x-5 rounded-xl bg-white p-1 overflow-x-auto'>
                    ...
                    {/* FOR DESKTOP VIEW */}
                    <button onClick={() => setSelectedButton('webdevelopment')} className={"bg-white " + (selectedButton === 'webdevelopment' ? 'text-black border-b-2 border-orange-500 tracking-wide' : 'text-lightgrey') + " pb-2 text-lg hidden sm:block"}>Web Development</button>
                    <button onClick={() => setSelectedButton('mobiledevelopment')} className={"bg-white " + (selectedButton === 'mobiledevelopment' ? 'text-black border-b-2 border-orange-500 tracking-wide' : 'text-lightgrey') + " pb-2 text-lg hidden sm:block"}>Mobile Development</button>
                    <button onClick={() => setSelectedButton('datascience')} className={"bg-white " + (selectedButton === 'datascience' ? 'text-black border-b-2 border-orange-500 tracking-wide' : 'text-lightgrey') + " pb-2 text-lg hidden sm:block"}>Data Science</button>
                    <button onClick={() => setSelectedButton('cloudcomputing')} className={"bg-white " + (selectedButton === 'cloudcomputing' ? 'text-black border-b-2 border-orange-500 tracking-wide' : 'text-lightgrey') + " pb-2 text-lg hidden sm:block"}>Cloud Computing</button>

                    {/* FOR MOBILE VIEW */}
                    <GlobeAltIcon onClick={() => setSelectedButton('webdevelopment')} width={70} height={70} className={"bg-white m-auto border" + (selectedButton === 'webdevelopment' ? 'border-b-2 border-orange fill-orange' : '') + " pb-2 block sm:hidden"} />
